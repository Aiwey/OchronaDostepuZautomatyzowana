<?php
function getFrom(&$source,&$idx,&$required,&$required_message){ //idx ->identyfikator
	if (isset($source[$idx])){
		return $source[$idx];
	} else {
...
function forwardTo($action_name){ //wywołaj front controler, zmien akcje
	global $action;
	$action = $action_name;
...
function redirectTo($action_name){ //przekieruj do
